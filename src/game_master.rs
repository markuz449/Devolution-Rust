...
fn write_title(game_state: &GameState, mut stdout: Out) -> Out {
    ...
    writeln!(
        ...
        "{}{}",
        color::Fg(color::Blue),
        format!("{:^1$}", game_state.planet, game_state.terminal_width)
    )
    .unwrap();
    writeln!(
        stdout,
        "{}{}",
        color::Fg(color::Red),
        format!("{:^1$}", game_state.title, game_state.terminal_width)
    )
    .unwrap();
    writeln!(stdout, "{}{}", color::Fg(color::LightWhite), style::Italic).unwrap();
    let start_message: String = format!(
        "{} {}{}{} {}",
        "Press",
...
fn write_character_creator(character: &Character, game_state: &GameState, mut stdout: Out) -> Out {
    ...
    if character.enter_name_active {
        ...
        writeln!(
            ...
            "\r{}{}\r",
            color::Fg(color::Blue),
            format!("{:^1$}", name_box, width + (name_box_offset * 2))
        )
        .unwrap();
        ...
        writeln!(
            ...
            format!("{:^1$}", name_box, width + name_box_offset)
        )
        .unwrap();
...
fn write_story(story: &StoryPage, game_state: &GameState, mut stdout: Out) -> Out {
    ...
    writeln!(
        ...
        "\r\n{}{}\r\n",
        color::Fg(color::Green),
        format!("{:^1$}", "Devolution", game_state.terminal_width)
    )
    .unwrap();
    ...
    if !story.game_over && !game_state.re_read_mode {
        writeln!(
            ...
            format!("{:^1$}", "Choices", game_state.terminal_width)
        )
        .unwrap();
        ...
        writeln!(
            ...
            "\r{}{}\r",
            color::Fg(color::Yellow),
            format!("{:^1$}", "Your Choice", game_state.terminal_width)
        )
        .unwrap();
    ...
    writeln!(
        ...
        "\r\n{}{}\r",
        color::Fg(color::Green),
        format!("{:^1$}", "To Quit, Press 'Esc'. For Help, Press 'h'", game_state.terminal_width)
    )
    .unwrap();
    writeln!(stdout, "{}", Hide).unwrap();
...
fn help(game_state: &GameState, mut stdout: Out) -> Out {
    ...
    writeln!(
        stdout,
        "\r{}\r",
        format!("{:^1$}", "Welcome to Devolution, a sci-fi adventure where you choose", width)
    )
    .unwrap();
    ...
    writeln!(
        ...
        "\r{}{}\r",
        color::Fg(color::Blue),
        format!("{:>1$}", header, (width / 3) + header.len())
    )
    .unwrap();
    writeln!(stdout, "{}{}", style::Italic, color::Fg(color::White)).unwrap();
