...
fn write_title(game_state: &GameState, mut stdout: Out) -> Out{
    write!(stdout, "{}", "\x1bc").unwrap();
    writeln!(stdout, "{}", format!("{:^1$}", game_state.planet.bold().blue(), game_state.terminal_width)).unwrap();
    writeln!(stdout, "{}", format!("{:^1$}", game_state.title.bold().red(), game_state.terminal_width)).unwrap();
...
fn write_story(story: &StoryPage, game_state: &GameState, mut stdout: Out) -> Out{
    write!(stdout, "{}", "\x1bc").unwrap();
    writeln!(stdout, "\r\n{}\r\n", format!("{:^1$}", "Devolution".bold().green(), game_state.terminal_width)).unwrap();
    writeln!(stdout, "{}", story.text.italic()).unwrap();

    if !story.game_over && !game_state.re_read_mode {
        writeln!(stdout, "\r{}\r", format!("{:^1$}", "Choices".bold().italic().yellow(), game_state.terminal_width)).unwrap();
        for i in 0..story.option_text.len(){
            if i == story.selection_num{
                write!(stdout, "\r{}\r", story.option_text[i].blue().bold()).unwrap();
            } else{
                write!(stdout, "\r{}\r", story.option_text[i].bold()).unwrap();
            }
        }
    } else if game_state.re_read_mode {
        writeln!(stdout, "\r{}\r", format!("{:^1$}", "Your Choice".bold().italic().yellow(), game_state.terminal_width)).unwrap();
        let choice_num: usize = game_state.story_path[game_state.previous_story_num].choice_num;
        write!(stdout, "\r{}\r", story.option_text[choice_num].red().bold()).unwrap();
    }

    writeln!(stdout, "\r\n{}\r", format!("{:^1$}", "To Quit, Press 'Esc'. For Help, Press 'h'".bold().italic().green(), game_state.terminal_width)).unwrap();
    writeln!(stdout, "{}", Hide).unwrap();
...
fn help(game_state: &GameState, mut stdout: Out) -> Out{
    ...
    writeln!(stdout, "").unwrap();
    writeln!(stdout, "\r{}\r", format!("{:^1$}", "Welcome to Devolution, a sci-fi adventure where you choose".italic(), width)).unwrap();
    writeln!(stdout, "\r{}\r", format!("{:^1$}", "how you progresses!".italic(), width)).unwrap();
    writeln!(stdout, "").unwrap();
    writeln!(stdout, "").unwrap();
    writeln!(stdout, "\r{}\r", format!("{:^1$}", controls, width)).unwrap();
    writeln!(stdout, "").unwrap();
    writeln!(stdout, "\r{}\r", format!("{:>1$}", header, (width / 5) + header.len())).unwrap();
    ...
    writeln!(stdout, "").unwrap();
    writeln!(stdout, "").unwrap();
    writeln!(stdout, "\r{}\r", format!("{:^1$}", exit, width)).unwrap();
